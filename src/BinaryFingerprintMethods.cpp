...
BinaryFingerprintMethods::BinaryFingerprintMethods() : lib_features_(NULL),
    query_features_(NULL), threads_(NULL), thread_data_(NULL), cutoff_(0.7),
    store_nns_(false), precision_(0.000001), n_threads_(1), verbosity_(0),
    ppm_search_thresh_(10.0) {
  setBlockSize(850);
}
...
bool BinaryFingerprintMethods::parseBinaryFingerprint(const std::string& fprint, vector<unsigned short>& features, unsigned int fp_type, const char* delim)
{
  features.clear();
...
void BinaryFingerprintMethods::createInvertedIndicesWithPrecMasses(const vector<pair<const vector<unsigned short>*, unsigned int> >& molecules, const std::vector<float>& prec_masses, InvertedIndices& target)
{
  vector<pair<const vector<unsigned short>*, unsigned int> > tmp;
  
  prec_mass_starts_.push_back(prec_masses[0]);
  unsigned int i;
  for (i = 0; i < molecules.size(); ++i) {
    tmp.push_back(molecules[i]);
    if ((i+1) % blocksize_ == 0) {
      prec_mass_starts_.push_back(prec_masses[i+1]);
      prec_mass_ends_.push_back(prec_masses[i]);
      target.push_back(createInvertedIndex(tmp));
      tmp.clear();
    }
  }
  ...
  if (tmp.size()) {
    prec_mass_ends_.push_back(prec_masses[i-1]);
    target.push_back(createInvertedIndex(tmp));
    tmp.clear();
  }
}
...
void BinaryFingerprintMethods::pairwiseSimilaritiesCutoffOMP(
    const unsigned int ii1_index, const unsigned int ii2_index, 
    unsigned short** cc_matrix, std::vector<PvalueTriplet>& triplets) {
  unsigned short* cc_matrix_row;
  ...
  if (ii1!=ii2)
  {
    ...
    for (unsigned int u=0; u!=ii1->n_molecules; ++u)
    {
      ...
      for (unsigned int v=0; v!=ii2_n_molecules; ++v)
      {
        ...
        if (coeff >= cutoff_) {
          PvalueTriplet t;
          t.scannr1.scannr = tmp_molecule_id;
          t.scannr2.scannr = ii2_molecules_base + v;
          t.pval = coeff;
...
void BinaryFingerprintMethods::pairwiseSimilaritiesCutoff(const unsigned int ii1_index, const unsigned int ii2_index, ThreadData* t_data)
{
  unsigned short* cc_matrix_row;
  unsigned short** cc_matrix = t_data->cc_matrix;
  ...
  std::vector<PvalueTriplet> triplets;
  ...
  bool append = true;
  BinaryInterface::write<PvalueTriplet>(triplets, t_data->outfile_name, append);
}
...
void BinaryFingerprintMethods::pairwiseSimilaritiesThread(const unsigned int thread_id)
{
  ...
  while (getNextComparisonIndex(index))
  {
    ...
    // If the last molecule of the row block has a precursor mass more than 10ppm away from the first molecule of the column block, we can skip the comparison
    if (prec_mass_ends_.size() > 0 && prec_mass_ends_[row_index] < prec_mass_starts_[col_index] * (1 - ppm_search_thresh_*1e-6)) continue;
    
    //std::cerr << row_index << " " << prec_mass_ends_[row_index] << " " << col_index << " " << prec_mass_starts_[col_index] << std::endl;
...
bool BinaryFingerprintMethods::pairwiseSimilaritiesOMPThread(LongSize index, std::vector<PvalueTriplet>& triplets) {
  LongSize row_index, col_index;
  ...
  // If the last molecule of the row block has a precursor mass more than 10ppm away from the first molecule of the column block, we can skip the comparison
  if (prec_mass_ends_.size() > 0 && prec_mass_ends_[row_index] < prec_mass_starts_[col_index] * (1 - ppm_search_thresh_*1e-6)) return false;
  
  unsigned short** cc_matrix = new unsigned short*[blocksize_ + 1];
  for (unsigned short j=0; j!=blocksize_ + 1; ++j) {
    cc_matrix[j] = new unsigned short[blocksize_ + 1];
    memset(cc_matrix[j], '\0', cc_matrix_size_);
  }
  ...
  // Calculate similarities
  pairwiseSimilaritiesCutoffOMP(row_index, col_index, cc_matrix, triplets);
...
void BinaryFingerprintMethods::cutoffSearchSimilarities(const unsigned int query_index, const unsigned int lib_index, unsigned short** cc_matrix, std::ofstream& outfile)
{
  unsigned short* cc_matrix_row;
...
void BinaryFingerprintMethods::cutoffSearchThread(const unsigned int thread_id)
{
  ThreadData *t_data = &thread_data_[thread_id];
  unsigned short** cc_matrix = t_data->cc_matrix;
  std::ofstream outfile(t_data->outfile_name.c_str());
...
bool BinaryFingerprintMethods::initInvertedIndicesWithPrecMasses(const std::vector<float>& prec_masses) {
  if (lib_features_ != NULL) {
    vector<pair<const vector<unsigned short>*, unsigned int> > tmp;
    for (unsigned int i=0; i!=lib_features_->size(); ++i) {
...
bool BinaryFingerprintMethods::cutoffSearchLibrary(const float cutoff, const std::vector<float>& prec_masses, vector<std::string>& tmp_file_names)
{
  cutoff_ = cutoff;
  ...
  if (!initInvertedIndicesWithPrecMasses(prec_masses)) {
    return false;
  }
  ...
  if (threads_ == NULL)
  {
    ...
    for (unsigned int i=0; i!=n_threads; ++i)
    {
      tmp_file_name = "tmp_file_" + boost::lexical_cast<std::string>(i);
...
bool BinaryFingerprintMethods::cutoffSearch(const float cutoff, const std::string& outfile_name)
{
  cutoff_ = cutoff;
...
//       tmp.push_back(make_pair((*query_features_)[i], 0));
  ...
  vector<std::string> tmp_file_names;
  ...
  {
    string line;
    std::ofstream result_file(outfile_name.c_str());
