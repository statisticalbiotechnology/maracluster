  ...
  class BinaryFingerprintMethods
  {
    ...
    public:
      ...
      */
       ...
       */
      static bool parseBinaryFingerprint(const std::string& fprint, std::vector<unsigned short>& features, unsigned int fp_type, const char* delim=",");
      ...
      /** 
       * MT: Set Tanimoto distance cutoff.
       * @param cutoff Tanimoto distance cutoff to use.
       */
      inline void setCutoff(const float cutoff) { cutoff_ = cutoff; }
       ...
       */
      inline void setNThreads(const int nThreads) { n_threads_ = nThreads; }
      
      /** 
       * MT: Set ppm threshold
       * @param ppm ppm threshold to use.
       */
      inline void setPpmThresh(const int ppmThresh) { ppm_search_thresh_ = ppmThresh; }
      //@}
       ...
       */
      bool cutoffSearch(const float sim_cutoff, const std::string& outfile_name);
      
      /**
       * MT: Initialize inverted indices together with precursor start and end masses for each block
       */
      bool initInvertedIndicesWithPrecMasses(const std::vector<float>& prec_masses);
      
      /**
       * MT: Perform similarity search for molecules of a target library with itself and keep pairs with similarity exceeding sim_cutoff.
       * @param sim_cutoff Similarity cutoff to apply for cutoff search, i.e. only keep similarities above cutoff.
       * @param file_name Output file name to store result pairs with similarities as space separated CSV file.
       */
      bool cutoffSearchLibrary(const float sim_cutoff, const std::vector<float>& prec_masses, std::vector<std::string>& tmp_file_names);
      
      /**
       * MT: Thread function for a single block pairwise comparison.
       * @param index Index of block comparison computed by arrayToUpperTriangluarMatrix().
       */
      bool pairwiseSimilaritiesOMPThread(LongSize index, std::vector<PvalueTriplet>& triplets);
    ...
    private:
      ...
      /**
       * MT: Vector of the first precursor mass for each block
       */
      std::vector<float> prec_mass_starts_;
      
      /**
       * MT: Vector of the last precursor mass for each block
       */
      std::vector<float> prec_mass_ends_;
      ...
      /**
       * MT: ppm threshold for candidate molecule pairs
       */
      double ppm_search_thresh_;
      ...
      /**
       * MT: Create InvertedIndices of same size for a set of input molecules.
       * @param molecules Feature vectors of molecules for which the InvertedIndices are created.
       * @param prec_masses Precursor masses of molecules for which the InvertedIndices are created.
       * @param target Reference to vector of InvertedIndices to store the created InvertedIndices (either lib_blocks or query_blocks).
       */
      void createInvertedIndicesWithPrecMasses(const std::vector<std::pair<const std::vector<unsigned short>*, unsigned int> >& molecules, const std::vector<float>& prec_masses, InvertedIndices& ii_target);
       ...
       */
      void cutoffSearchSimilarities(const unsigned int query_index, const unsigned int lib_index, unsigned short** cc_matrix, std::ofstream& outfile);
      ...
      /**
       * MT: Pairwise similarity calculation which implements calculation of a similarity matrix.
       * @param ii1_index Position of first InvertedIndex to be compared.
       * @param ii2_index Position of second InvertedIndex to be compared.
       * @param t_data Pointer to thread data structure.
       */
      void pairwiseSimilaritiesCutoffOMP(const unsigned int ii1_index, const unsigned int ii2_index, 
       ...
       * @param t_data Pointer to thread data structure.
       */
      void pairwiseSimilaritiesCutoff(const unsigned int ii1_index, const unsigned int ii2_index, ThreadData* t_data);
