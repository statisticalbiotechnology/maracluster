...
if [ ! -d Qt-dynamic ]; then
  ...
  if [ ! -f ${linux_qt}.tar.xz ]; then
    wget --no-check-certificate --no-verbose https://download.qt.io/archive/qt/5.9/5.9.9/submodules/${linux_qt}.tar.xz
  fi
...
if version_lt $(cmake --version | head -n1 | cut -f3 -d ' ') "3.5"; then
  wget --no-check-certificate --no-verbose https://github.com/Kitware/CMake/releases/download/v3.13.3/cmake-3.13.3-Linux-x86_64.sh
